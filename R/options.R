...
#' \describe{
#'  \item{\option{doFuture.foreach.export}:}{Specifies to what extent the \code{.export} argument of \code{\link[foreach]{foreach}()} should be respected or if globals should be automatically identified.  If \code{"automatic"}, the future framework will be used to automatically identify globals.  If \code{".export"}, then the globals specified by the \code{.export} argument will be used "as is".  If \code{"automatic-unless-.export"}, then globals are automatic identified if argument \code{.export} is missing or \code{NULL}.  (Default: \code{"automatic-unless-.export"})}
#' }
#'
